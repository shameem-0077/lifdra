...
const TestimonialModal = ({
	...
	const nextFunction = (data) => {
		testimonials.map((item, index) => {
			if (item.id === data.id && index != testimonials.length - 1) {
				setModalData(testimonials[index + 1]);
			}
		});
	};

	const prevFunction = (data) => {
		testimonials.map((item, index) => {
	...
	return (
		<BackContainer style={{ transform: isModal && "scale(1,1)" }}>
			...
			<Modal>
				<PrevArrow onClick={() => prevFunction(data)}>
					<img
				...
				<NextArrow onClick={() => nextFunction(data)}>
					<img
			...
			> */}
				...
				<Close onClick={handleModal}>
					<img
						src={require("../../../../../../assets/images/web/close.svg")}
						alt=""
					/>
				</Close>
