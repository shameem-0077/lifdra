...
function OptModal({
					...
					if (window.tidioChatApi) {
						if (viewPhone)
							window.tidioChatApi.messageFromVisitor(
								`I'm facing an issue with the OTP related to School Scientist, my phone number is ${viewPhone}.`
							);
						else {
							window.tidioChatApi.messageFromVisitor(
								`I'm facing an issue with the OTP related to School Scientist.`
							);
						}
					}
				});
		})();
	}, [viewPhone, window.tidioChatApi]);
	...
	const renderSubmit = () => {
		...
		formData.append("phone", viewPhone);
				...
				<Modal>
					...
					<Container>
						...
						<IssueContainer className="g-medium">
							If you are facing any issues to get the
							OTP,{" "}
							<Links
								href={`https://wa.me/${+918589998874}`}
								className="action chat-button"
								target="_blank"
							>
