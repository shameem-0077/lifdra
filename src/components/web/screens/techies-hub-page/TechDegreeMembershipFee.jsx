...
function TechDegreeMembershipFee() {
  ...
  const steps = [
    {
      ...
      description:
        "To get access to Steyp's Tech Degree program the candidate have to clear SAT.",
    },
    {
      id: 2,
      title: "Selection ",
    ...
    },
    {
      ...
      description:
        "Candidates can download the digital certificate through the platform after completing each profession.",
    },
    {
      id: 5,
      title: "Placement",
      abbr: "",
      isabbr: false,
      icon: "https://s3.ap-south-1.amazonaws.com/talrop.com-react-assets-bucket/assets/images/06-05-2022/placement.svg",
      description:
  ...
  return (
    <Container>
      <Cover className="wrapper">
        <LeftSection>
          <Title>Tech Degree Fee</Title>
          <Description>
            {/* <span> Upload your valid ID card </span>and get <small>50% </small>off on
                        your package. */}
            Get admission by qualifying <HighlightedText>SAT</HighlightedText>{" "}
            Exam and now get placed as IT engineers through our program
          </Description>
          ...
          {steps.map((data) => {
            return (
              <StepCard key={data.id}>
                ...
                >
                  {data.button ? (
                    <ApplyNow onClick={handleModal}>{data.title}</ApplyNow>
                  ) : (
                    <CardTitle>
                      {data.title} {data.isabbr ? <Span>{data.abbr}</Span> : ""}
                    </CardTitle>
                  )}
                  <DetailsBox>
                    <Details>{data.description}</Details>
                    {data.isabbr ? (
                      <KnowMoreBtn to="/sat/">Know more</KnowMoreBtn>
                    ) : (
                      ""
                    )}
                  </DetailsBox>
